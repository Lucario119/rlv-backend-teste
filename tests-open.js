...
export default () => {
  group('Dia do Trabalhador', function () {
    ...
    check(query1.json(), {
      'é Dia do Trabalhador no Oiapoque': (obj) =>
        obj.name === 'Dia do Trabalhador',
    });
    ...
    check(query2.json(), {
      'é Dia do Trabalhador no Chuí': (obj) =>
        obj.name === 'Dia do Trabalhador',
    });
  ...
  group('Consciência Negra RJ', function () {
    ...
    let register = http.put(`${BASE_URL}/feriados/33/11-20/`, payload, {
      headers: HEADERS,
    });
    check(register, {
      'cadastro retorna status 200 ou 201': (r) =>
    ...
    check(query1.json(), {
      'consulta no estado retorna o dia da Consciência Negra': (obj) =>
        obj.name === 'Consciência Negra',
    });
    ...
    check(query2.json(), {
      'consulta na capital retorna o dia da Consciência Negra': (obj) =>
        obj.name === 'Consciência Negra',
    });
    ...
    check(del1, {
      'tentar remover o feriado na capital retorna status 403': (r) =>
        r.status === 403,
    });
  ...
  group('Sexta-Feira Santa', function () {
    ...
    check(query.json(), {
      'é Sexta-Feira Santa em São Luís': (obj) =>
        obj.name === 'Sexta-Feira Santa',
    });
  });
  group('Corpus Christi Ouro Preto', function () {
    // Cadastro de feriado móvel municipal
    let register = http.put(`${BASE_URL}/feriados/3146107/corpus-christi/`, {
      headers: HEADERS,
    });
    check(register, {
      'cadastro retorna status 200 ou 201': (r) =>
    ...
    check(query1.json(), {
      'é Corpus Christi em Ouro Preto em 2020': (obj) =>
        obj.name === 'Corpus Christi',
    });
    ...
    check(query2.json(), {
      'é Corpus Christi em Ouro Preto em 2021': (obj) =>
        obj.name === 'Corpus Christi',
    });
  });
};
